(** This file is a FAILED attempt at using locally nameless: it shows why
    cofinite quantification is not enough (see the comments in the proof
    of [reduce_stack_correct]). In particular cofinite quantification in the
    rule for reduction under lambda abstractions [red_lam] is too restrictive:
    we need both a universally quantified version and an existentially quantified
    version for the proof to go through.

    The notations and basic lemmas in this file are inspired by
    Arthur Charguéraud's paper 'The Locally Nameless Representation'.
*)
    ...
    * remember (fresh_name ctx) as y. cbn.
      ...
      forward IHfuel.
      {
        (* Here we want to apply [H] which corresponds to the rule for lambda in [well_scoped].
           ...
           we have an _arbitrary_ set [L] and we need to show that the name
           [y] we got from [fresh_name] is not in [L].
           We would need the rule for [well_scoped_lam] to have a universally quantified
           premise. *)
        apply H. admit.
      }
